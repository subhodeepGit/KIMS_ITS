...
class POConsumable(Document):
	def before_validate(self):
		for z in frappe.get_all("Details of Invoices and PO",{"parent":self.name},["invoice_receival_no"]):
			...
			if self.workflow_state=="Rejected and Transfer":
				...
				if name!="":
					...
					if emp_data:
						...
						frappe.db.commit()
						frappe.db.sql(""" update `tabPO Consumable` set document_status="%s" where name="%s" """%("Rejected and Transfer",self.name))
						frappe.db.commit()
