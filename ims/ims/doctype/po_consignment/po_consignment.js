...
frappe.ui.form.on("PO Consignment", {
	onload:function(frm){
		...
		if(frm.is_new()==1 || frm.doc.workflow_state=="Draft" || frm.doc.workflow_state=="Verify and Save"){
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "invoice_receival_no", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "hospital_margin_note_no", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "po", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "po_attachment", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "document_receiving_date", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "hospital_margin_note_date", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "hospital_margin_amount", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "patient_name", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "mrn_no", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "document_no", cur_frm.doc.name);
			df_rate.read_only = 0;
			var df_rate = frappe.meta.get_docfield("Credit Note and PO", "document_date", cur_frm.doc.name);
			df_rate.read_only = 0;
			frm.refresh_fields();
			}
			else{
		...
		if(frm.doc.workflow_state=="Draft" || frm.doc.workflow_state=="Verify and Save"){
			frm.set_df_property("details_of_invoices_credit_note_and_po", "cannot_add_rows", false);
			frm.set_df_property("details_of_invoices_credit_note_and_po", "cannot_delete_rows", false);
			}
			else{
			frm.set_df_property("details_of_invoices_credit_note_and_po", "cannot_add_rows", true);
			frm.set_df_property("details_of_invoices_credit_note_and_po", "cannot_delete_rows", true);
			}
		if(frm.is_new()==1){
