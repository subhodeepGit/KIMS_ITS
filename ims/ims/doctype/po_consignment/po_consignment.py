...
class POConsignment(Document):
	def before_validate(self):
		for z in frappe.get_all("Credit Note and PO",{"parent":self.name},["invoice_receival_no"]):
			...
			if self.workflow_state=="Rejected and Transfer":
				...
				if name!="":
					...
					if emp_data:
						...
						frappe.db.sql(""" update `tabPO Consignment` set workflow_state="%s" where name="%s" """%(check,self.name))
						frappe.db.commit()
						frappe.db.sql(""" update `tabPO Consignment` set document_status="%s" where name="%s" """%("Rejected and Transfer",self.name))
						frappe.db.commit()
...
def get_table_attachments():
	...
	for t in frappe.get_all("DocField",{"parent": "Credit Note and PO","fieldtype":"Attach"},["fieldname","mandatory_depends_on"]):
		...
		for t in self.get("third_party_verification"):
			if t.final_status=="Open" and t.status_of_verification==None:
				...
				if emp_data:
					...
					for j in self.get("details_of_invoices_credit_note_and_po"):
						ref_party_doc.append("third_party_verification_child",{
							"document_no":j.document_no,
							"invoice_no":j.inv_no,
							...
							"mrn_no":j.mrn_no,
							"hospital_margin_amount":j.hospital_margin_amount,
							"to_pay":j.to_pay,
							"hospital_margin_amount":j.hospital_margin_amount,
							"document_attachment":j.others,
							"po_attachment_attachment":j.po,
							"bill_summary_attadelivery_challanchment":j.bill_challan,
...
def status_update(self):
		...
		for t in self.get("details_of_invoices_credit_note_and_po"):
			if workflow_status!="Cancelled":
				...
				frappe.db.set_value("Invoice Receival",t.invoice_receival_no,"type_of_note_sheet","PO Consignment")
			else:
