			...
			if self.workflow_state=="Rejected and Transfer":
				...
				if name!="":
					...
					if emp_data:
						...
						frappe.db.sql(""" update `tabNon PO Non Contract` set workflow_state="%s" where name="%s" """%(check,self.name))
						frappe.db.commit()
						frappe.db.sql(""" update `tabNon PO Non Contract` set document_status="%s" where name="%s" """%("Rejected and Transfer",self.name))
						frappe.db.commit()
...
def status_update(self):
		...
		for t in self.get("details_of_enclosed_bills"):
			if workflow_status!="Cancelled":
				...
				frappe.db.set_value("Invoice Receival",t.invoice_receival_no,"type_of_note_sheet","Non PO Non Contract")
			else:
