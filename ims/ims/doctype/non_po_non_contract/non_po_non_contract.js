...
frappe.ui.form.on("Non PO Non Contract", {
	...
	if(frm.is_new()==1 || frm.doc.workflow_state=="Draft" || frm.doc.workflow_state=="Verify and Save"){
		var df_rate = frappe.meta.get_docfield("Details Enclosed Bills", "invoice_receival_no", cur_frm.doc.name);
		df_rate.read_only = 0;
		var df_rate = frappe.meta.get_docfield("Details Enclosed Bills", "po_attachment_attachment", cur_frm.doc.name);
		df_rate.read_only = 0;
		var df_rate = frappe.meta.get_docfield("Details Enclosed Bills", "po_attachment", cur_frm.doc.name);
		df_rate.read_only = 0;
		frm.refresh_fields();
		}
		else{
	...
	if(frm.doc.workflow_state=="Draft" || frm.doc.workflow_state=="Verify and Save"){
		...
		frm.set_df_property("details_of_enclosed_bills", "cannot_add_rows", true);
		frm.set_df_property("details_of_enclosed_bills", "cannot_delete_rows", true);
		}
